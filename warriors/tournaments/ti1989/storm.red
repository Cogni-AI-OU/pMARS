...
ptr      dat     #0,           #8              ; puntatore per il bombardamento
...
ct       djn     loop,         #988            ; ci si arresta prima di colpirsi
         jmp     phase2,       0               ; e ci si copia più in là
poison   jmp     @0,           0               ; ecco la bomba alla cowboy
entrance sub     #1,           slaves          ; ogni processo catturato decrementa un
prison   spl     entrance,     0               ; contatore e cerca di generare altri 
         jmn     prison,       slaves          ; prigionieri. Se sono 64 si suicidano
slaves   dat     #0,           #64             ; contatore per i prigionieri
init     add     #11,          to              ; processo ausiliario per la reinizializ-
         mov     new,          @to             ; zazione del contatore dopo la copia
new      dat     #0,           #64             ; allo scopo si usa questa dat
unused   jmp     entrance,     0               ; qui cadrà una bomba ...
...
copy     mov     @ptr,         <to             ; il classico ciclo di copia...
         djn     copy,         ptr             ; ...
         spl     @to,          0               ; si lancia la nuova copia
to       jmp     init,         0               ; e si avvia il processo ausiliario
