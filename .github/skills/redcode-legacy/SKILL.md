...
- **Strict ICWS'88 (`-8` flag) Compliance**:
  ...
  - **Immediate Operand Restrictions**: Some instructions (e.g., `CMP`, `JMN`) do not support immediate values (`#`) in the B-field.
...
- **Automated Parameter Extraction**:
  ...
  - Supported parameters: `CORESIZE` (`-s`), `MAXLENGTH` (`-l`), `MAXPROCESSES` (`-p`), `MAXCYCLES` (`-c`), `MINDISTANCE` (`-d`).
...
- **]enigma[ (Round 7)**: Non-standard `<<` addressing modes normalized to `<` for ICWS'88 compliance.
